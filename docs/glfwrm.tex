...
% API Version: 2.4
% $Id: glfwrm.tex,v 1.2 2003-11-04 22:14:46 marcus256 Exp $
%-------------------------------------------------------------------------
...
\begin{description}
\item [\textit{cbfun}]\ \\
  Pointer to a callback function which will be called every time a key is
  ...
  ISO~8859-1 character or a special key identifier (see \textbf{glfwGetKey}
  for more information), and action is either GLFW\_PRESS or GLFW\_RELEASE.
...
\begin{description}
\item [\textit{cbfun}]\ \\
  Pointer to a callback function which will be called every time a
  ...
  Where \textit{functionname} is the name of the callback function,
  \textit{character} is a Unicode (ISO~10646) character, and action is
