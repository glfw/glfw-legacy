...
% API Version: 2.5
% $Id: glfwrm.tex,v 1.18 2004-07-09 21:20:30 marcus256 Exp $
%-------------------------------------------------------------------------
...
\item MS--DOS\textsuperscript{\textregistered}\footnotemark[\value{footnote}]
\end{itemize}
...
As for user input, it is possible to recieve information about window
...
In C language, a typical event loop might look like this:
...
The loop in the example only waits until the window is closed, and then
aborts the loop (possibly continuing by terminating the application).
...
%-------------------------------------------------------------------------
\subsection{glfwSetWindowPaintCallback}
...
\begin{refdescription}
The function selects which function to be called upon a window paint
...
\begin{refnotes}
Window paint events are recorded continuously, but only reported when
\textbf{glfwPollEvents}, \textbf{glfwWaitEvents} or
\textbf{glfwSwapBuffers} is called.
\end{refnotes}
