...
% GLFW Users Guide
% API Version: 2.4
% $Id: glfwug.tex,v 1.6 2003-11-15 21:52:54 marcus256 Exp $
%-------------------------------------------------------------------------
...
GLFW\_PRESS (or 1) if the corresponding mouse button is held down,
otherwise it will return GLFW\_RELEASE (or 0).
...
\textbf{glfwEnable} with the argument GLFW\_STICKY\_MOUSE\_BUTTONS. To
disable sticky mouse buttons, call \textbf{glfwDisable} with the same
argument.
...
joystick, \textit{param} should be GLFW\_JOYSTICK\_AXES or
GLFW\_JOYSTICK\_BUTTONS, respectively.
...
the pixel format and data storage flags are handled automatically.

The \textit{flags} argument can be used to control how the texture is
loaded. If \textit{flags} is zero, the origin of the texture will be the
lower left corner, and only one mipmap level is loaded. If \textit{flags}
is GLFW\_ORIGIN\_UL\_BIT the origin of the texture will be the upper left
corner. If \textit{flags} is GLFW\_BUILD\_MIPMAPS\_BIT, all mipmap levels
will be generated and uploaded to texture memory. To combine
GLFW\_ORIGIN\_UL\_BIT and GLFW\_BUILD\_MIPMAPS\_BIT, or them together
(i.e. like this: \texttt{GLFW\_ORIGIN\_UL\_BIT | GLFW\_BUILD\_MIPMAPS\_BIT}).

Here is an example of how to upload a texture from a file to \OpenGL\
texture memroy, and configure the texture for trilinear interpolation
(assuming an \OpenGL\ window has been opened successfully):

\begin{lstlisting}
    GLuint  texid;

    // Generate texture object for one texture
...
and GLFW\_NO\_RESCALE\_BIT. GLFW\_ORIGIN\_UL\_BIT works as described for
the \textbf{glfwLoadTexture2D} function. If the GLFW\_NO\_RESCALE\_BIT
flag is set, the image will not be rescaled to the closest larger
$2^m~\times~2^n$ resolution, which is otherwise the default action for
images with non-power-of-two dimenstions.
...
Multi processor platforms are becoming increasingly common, and the price
to pay is generally not much higher than for a single processor system.

A multi processor system is especially appealing if you consider that in
general, for the price of the fastest processor available you can get two
processors of a model which perform only slightly worse.
...
The argument \textit{fun} is a pointer to a function which will be
...
That was easy! Multi threading is easy!
...
Later on you will learn how to do advanced thread synchronization using
condition variables, which let threads wait for certain conditions before
